...
data loopso;
	...
	if find(hic3_desc,'LOOP DIURETIC') then loopso=1;

	if loopso=1;
	
run;

proc sort data=loopso; by ndc; run;
	
/************* loopso Pull **************/

%macro loopsopull(year,prev_year,merge,set);
...
data pde&year._1;
	merge pde&year. (in=a) loopso (in=b);
	by ndc;
	if a and b;
run;
...
data pde&year._2;
	&set. set pde&year._1;
	&merge. merge pde&year._1 (in=a) reppoly.loopso_&prev_year.p (in=b keep=bene_id loopso_push_&prev_year.);
	by bene_id;
...
data pde&year._3;
	* the following steps create a variable called uplag_srvc_dt which is the equivalent of [_n-1] in loopsoa;
...
data pde&year._4;
	...
	* pushstock is the accumulated stock of extra pills. Capped at 10;
	...
	&merge. if first.bene_id then pushstock=sum(pushstock,loopso_push_&prev_year.);
	...
	* The methodology below will do the following:
	...
	array loopso_a [*] loopso_a1-loopso_a365;
	do i=1 to 365;
		if doy_srvc_dt <= i <= sum(doy_srvc_dt,dayssply2) and loopso=1 then loopso_a[i]=1;
	end;
...
proc means data=pde&year._4 nway noprint;
	class bene_id;
	output out=pde&year._5 (drop=_type_ rename=_freq_=loopso_clms_&year.)
	sum(dayssply)=loopso_filldays_&year. min(srvc_dt)=loopso_minfilldt_&year. max(srvc_dt)=loopso_maxfilldt_&year.
	max(loopso_a1-loopso_a365 earlyfill_push lastfill_push loopso)=;
run;

data pde&year._6;
	set pde&year._5;
	loopso_fillperiod_&year.=max(loopso_maxfilldt_&year.-loopso_minfilldt_&year.+1,0);
run;
...
data pde&year._7;
	...
	* snf_push is the extra days added for SNF days concurrent with drug days;
	...
	do i=1 to 365;
		if loopso_a[i]=1 and snf_a[i]=1 then snf_push=snf_push+1;

		* if the loopso_a is 1, do nothing (already added it to the snf_push);
		* if the loopso_a spot is empty, then filling in with snf_push;
		if snf_push>0 and loopso_a[i]=. then do;
			loopso_a[i]=1;
			snf_push=snf_push-1;
...
data pde&year._8;
	...
	* ips_push is the extra days added for ip days concurrent with drug days;
	...
	do i=1 to 365;
		...
		if loopso_a[i]=1 and ips_a[i]=1  then ips_push=ips_push+1;

		* if the loopso_a is 1, do nothing (already added to the ips_push);
		* if the loopso_a spot is not full then adding in the ips_push;
		if ips_push>0 and loopso_a[i]=. then do;
			loopso_a[i]=1;
			ips_push=ips_push-1;
...
data pde&year._9;
	...
	* The array is filled using dayssply2, which includes adjustments for early fills.
	...
	loopso_pdays_&year=max(min(sum(of loopso_a1-loopso_a365),365),0);
...
data pde&year._10;
  ...
  loopso_push_&year.=max(lastfill_push,inyear_push);
  if loopso_push_&year.<0 then loopso_push_&year.=0;
  if loopso_push_&year.>90 then loopso_push_&year.=90;
  keep bene_id loopso:;
run;

* create perm;
data reppoly.loopso_&year.p;
	set pde&year._10;
run;
...
/*
%loopsopull(2006,2005,*,);
%loopsopull(2007,2006,,*);
%loopsopull(2008,2007,,*);
%loopsopull(2009,2008,,*);
%loopsopull(2010,2009,,*);
%loopsopull(2011,2010,,*);
%loopsopull(2012,2011,,*);
%loopsopull(2013,2012,,*);
*/
%loopsopull(2014,2013,,*);

/*********** Merge loopso 07-14 **********/
data loopso_0714;
	merge reppoly.loopso_2007p (in=b)
				reppoly.loopso_2008p (in=c)
				reppoly.loopso_2009p (in=d)
				reppoly.loopso_2010p (in=e)
				reppoly.loopso_2011p (in=f)
				reppoly.loopso_2012p (in=g)
				reppoly.loopso_2013p (in=h)
				reppoly.loopso_2014p (in=i);
	by bene_id;
	yloopso2007=b;
	yloopso2008=c;
	yloopso2009=d;
	...
	* timing variables;
	array yloopso [*] yloopso2007-yloopso2014;
	array yloopsodec [*] yloopso2014 yloopso2013 yloopso2012 yloopso2011 yloopso2010 yloopso2009 yloopso2008 yloopso2007 ;
	do i=1 to dim(yloopso);
		if yloopso[i]=1 then loopso_lastyoo=i+2006;
		if yloopsodec[i]=1 then loopso_firstyoo=2015-i;
	end;

	loopso_yearcount=loopso_lastyoo-loopso_firstyoo+1;

	* utilization variables;
	array util [*] loopso_fillperiod_2007-loopso_fillperiod_2014 loopso_clms_2007 - loopso_clms_2014
		loopso_filldays_2007-loopso_filldays_2014 loopso_pdays_2007-loopso_pdays_2014;
	...
	* total utilization;
	loopso_clms=sum(of loopso_clms_2007-loopso_clms_2014);
	loopso_filldays=sum(of loopso_filldays_2007-loopso_filldays_2014);
	loopso_pdays=sum(of loopso_pdays_2007-loopso_pdays_2014);

	* timing variables;
	loopso_minfilldt=min(of loopso_minfilldt_2007-loopso_minfilldt_2014);
	loopso_maxfilldt=max(of loopso_maxfilldt_2007-loopso_maxfilldt_2014);

	loopso_fillperiod=loopso_maxfilldt - loopso_minfilldt+1;

	loopso_pdayspy=loopso_pdays/loopso_yearcount;
	loopso_filldayspy=loopso_filldays/loopso_yearcount;
	loopso_clmspy=loopso_clms/loopso_yearcount;
...
* create perm;
data reppoly.loopso_0714p;
	set loopso_0714;
run;

* checks;
proc means data=loopso_0714; run;
